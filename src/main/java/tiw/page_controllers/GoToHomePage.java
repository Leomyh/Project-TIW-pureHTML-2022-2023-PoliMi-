...
public class GoToHomePage extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (last_seen != null && last_seen.size() > 0) {
			// Chronology is inserted in session
			ctx.setVariable("last_seen", last_seen);
		} else {
			response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Database Error");
		}

		try {
			// Initialize Cart in Server (Java): useful for GetProducts
			CartManager.getMyUserCart(user.getEmail(), connection, request);
		} catch (SQLException e) {
			response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Database Error");
		}
